...
export interface CoreState {
    ...
    colorScheme: { primary: [number, number, number], secondary: [number, number, number], background: [number, number, number] };
    needsShaderUpdate: boolean;
    _dirtyUniforms: Set<string>;
    isRendering: boolean;
    animationFrameId: number | null;
    shaderProgramName: string;
    callbacks: { onRender: ((state: any) => void) | null, onError: ((err: any) => void) | null };
    [key: string]: any; // For loose access
}
...
export default class HypercubeCore {
    ...
    constructor(canvas: HTMLCanvasElement, shaderManager: ShaderManager, options: Partial<CoreState> = {}) {
    ...
    _markUniformDirty(stateKey: string) { 
    ...
    _updateShaderIfNeeded() { 
        ...
        // FIX: Use null coalescing operator ?? because '0' is falsy but valid location
        this.aPositionLoc = this.shaderManager.getAttributeLocation('a_position') ?? -1; 
        
        if (this.aPositionLoc < 0) { 
            console.warn(`Attr 'a_position' missing.`); 
        } else { 
            try { this.gl.enableVertexAttribArray(this.aPositionLoc); } 
            catch (e) { console.error(`Enable attr error:`, e); this.aPositionLoc = -1; } 
        } 
        this._markAllUniformsDirty(); 
    ...
    updateParameters(newParams: Partial<CoreState>) { 
        ...
        for (const key in newParams) { 
            ...
            if (typeof oldValue === 'object' && oldValue !== null && !Array.isArray(oldValue)) { 
                ...
                this._markUniformDirty(key); 
                if (key === 'geometryType' || key === 'projectionMethod') { shaderNeedsUpdate = true; } 
            } 
        } 
    ...
    _setUniforms() {
        ...
        if (timeLoc) gl.uniform1f(timeLoc, this.state.time); else dirty.add('u_time');
        
        const uniformsToRetry = new Set<string>();
        ...
        dirty.forEach(name => { 
            ...
            if (loc !== null) { 
                ...
                } catch (e) { console.error(`Error setting uniform '${name}':`, e); } 
            } else { 
    ...
    _render(timestamp: number) { 
        ...
        if (this.state.needsShaderUpdate) { 
            if (!this._updateShaderIfNeeded()) { return; } 
        } 
    ...
    start() { 
        if (this.state.isRendering) return; 
        if (!this.gl || this.gl.isContextLost()) { console.error(`Cannot start, WebGL context invalid.`); return; } 
        ...
        if (this.state.needsShaderUpdate) { 
            if (!this._updateShaderIfNeeded()) { console.error(`Initial shader update failed.`); this.state.isRendering = false; return; } 
        } else if (this.aPositionLoc < 0) { 
            // FIX: Use null coalescing operator ??
            this.aPositionLoc = this.shaderManager.getAttributeLocation('a_position') ?? -1; 
            if (this.aPositionLoc < 0) { console.error(`Attr 'a_position' invalid.`); this.state.isRendering = false; return; } 
            try { this.gl.enableVertexAttribArray(this.aPositionLoc); } 
            catch (e) { console.error("Enable attr error:", e); this.state.isRendering = false; return; } 
        } 
    ...
    dispose() { 
        ...
        console.log(`Disposing HypercubeCore (${name})...`); 
        this.stop(); 
        if (this.gl && !this.gl.isContextLost()) { 
            try { 
                if (this.quadBuffer) this.gl.deleteBuffer(this.quadBuffer); 
                if (this.shaderManager?.dispose) { this.shaderManager.dispose(); } 
                const loseCtx = this.gl.getExtension('WEBGL_lose_context'); 
                loseCtx?.loseContext(); 
            } catch(e) { console.warn(`WebGL cleanup error:`, e); } 
        } 
        this.quadBuffer = null; 
        // @ts-ignore
        this.gl = null; 
