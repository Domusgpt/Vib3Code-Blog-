/* core/ShaderManager.ts */
import GeometryManager from './GeometryManager';
import ProjectionManager from './ProjectionManager';

interface ShaderOptions {
    baseVertexShaderName: string;
    baseFragmentShaderName: string;
}
...
export default class ShaderManager {
    ...
    shaderSources: Record<string, { source: string, type: number }>;
    compiledShaders: Record<string, WebGLShader | null>;
    programs: Record<string, WebGLProgram | null>;
    uniformLocations: Record<string, Record<string, WebGLUniformLocation | null>>;
    attributeLocations: Record<string, Record<string, number | null>>;
    currentProgramName: string | null;

    constructor(gl: WebGLRenderingContext, geometryManager: GeometryManager, projectionManager: ProjectionManager, options = {}) { 
    ...
    _mergeDefaults(options: any): ShaderOptions { return { baseVertexShaderName: 'base-vertex', baseFragmentShaderName: 'base-fragment', ...options }; }
    ...
    _registerShaderSource(name: string, source: string, type: number){ this.shaderSources[name] = { source, type }; }
    
    _compileShader(shaderIdentifier: string, source: string, type: number): WebGLShader | null { 
        if (this.compiledShaders[shaderIdentifier]) { return this.compiledShaders[shaderIdentifier]; } 
    ...
    _logShaderSourceWithError(source: string, errorLog: string) { 
        const lines=source.split('\n'); 
        const match=errorLog.match(/ERROR:\s*\d+:(\d+):/); 
        let errLine=match?parseInt(match[1],10):-1; 
        console.error("--- Shader Source ---"); 
        lines.forEach((line, i)=>{const p=(i+1===errLine)?">> ": "   "; console.error(p+(i+1).toString().padStart(3)+": "+line);}); 
        console.error("--- Shader Source End ---"); 
    }
    
    _createProgram(programName: string, vertexShader: WebGLShader, fragmentShader: WebGLShader): WebGLProgram | null { 
        ...
        if (!program) { console.error(`Failed create program '${programName}'.`); return null; } 
    ...
    createDynamicProgram(programName: string, geometryTypeName: string, projectionMethodName: string) { 
        ...
        if (!geom || !proj) { console.error(`Geom/Proj provider missing.`); return null; } 
    ...
    useProgram(programName: string | null) { 
        if (programName === null) { 
            if (this.currentProgramName !== null) { 
                try { this.gl.useProgram(null); } catch(e){} 
                this.currentProgramName = null; 
            } 
            return true; 
        } 
        ...
        if (program) { 
            ...
            if (currentGLProgram !== program) { 
                try { this.gl.useProgram(program); } 
                catch(e) { console.error(`useProgram failed for ${programName}`, e); return false;} 
            } 
    ...
    getUniformLocation(name: string) { 
        if (!this.currentProgramName || !this.programs[this.currentProgramName]) { return null; } 
    ...
    dispose() { 
        console.log("Disposing ShaderManager..."); 
        if (!this.gl) return; 
        try { this.gl.useProgram(null); } catch(e) {} 
        for (const name in this.programs) { 
            if (this.programs[name]) { 
                ...
                try { 
                    const s = this.gl.getAttachedShaders(p as WebGLProgram); 
                    s?.forEach(sh=>this.gl.detachShader(p as WebGLProgram,sh)); 
        ...
        for (const name in this.compiledShaders) { 
            if(this.compiledShaders[name]) { 
                try {this.gl.deleteShader(this.compiledShaders[name] as WebGLShader);} catch(e){} 
            } 
        } 
        this.compiledShaders={}; 
        this.shaderSources={}; 
